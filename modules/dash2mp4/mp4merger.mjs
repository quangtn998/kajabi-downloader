...
import {Mp4Sample} from '../hls/Mp4Sample.mjs';
...
export class MP4Merger extends EventEmitter {
  ...
  async pushFragment(track, fragData) {
    console.log('🔧 pushFragment() called for track:', track?.type);
    const entry = await fragData.getEntry();
    console.log('🔧 pushFragment() - getEntry() completed');
    const blob = await entry.getData();
    console.log('🔧 pushFragment() - getData() completed, blob size:', blob.size);
    const data = await BlobManager.getDataFromBlob(blob, 'arraybuffer');
    console.log('🔧 pushFragment() - BlobManager.getDataFromBlob() completed, data size:', data.byteLength);
    data.fileStart = 0;
    ...
    console.log('🔧 pushFragment() - About to call mp4boxfile.appendBuffer()');
    mp4boxfile.appendBuffer(data);
    console.log('🔧 pushFragment() - appendBuffer() completed');
    mp4boxfile.flush();
    console.log('🔧 pushFragment() - flush() completed');
  ...
  setup(videoDuration, videoInitSegment, audioDuration, audioInitSegment) {
    console.log('🔧 MP4Merger.setup() called with videoDuration:', videoDuration, 'audioDuration:', audioDuration);
    console.log('🔧 MP4Merger.setup() videoInitSegment size:', videoInitSegment?.byteLength, 'audioInitSegment size:', audioInitSegment?.byteLength);
    ...
    if (videoDuration) {
      console.log('🔧 MP4Merger.setup() - Processing video init segment');
      // Check if this is a WebM file (starts with 0x1A)
      const firstByte = new Uint8Array(videoInitSegment)[0];
      console.log('🔧 MP4Merger.setup() - Video init first byte:', firstByte, '(WebM signature is 0x1A = 26)');
      if (firstByte === 0x1A) {
        console.log('🔧 MP4Merger.setup() - Detected WebM init segment, skipping MP4Box processing');
        throw new Error('Video is not an mp4!');
      }
      
      const file = MP4Box.createFile(false);
      videoInitSegment.fileStart = 0;
      console.log('🔧 MP4Merger.setup() - About to appendBuffer video init segment');
      file.appendBuffer(videoInitSegment);
      console.log('🔧 MP4Merger.setup() - Video init appendBuffer completed');
      file.flush();
      console.log('🔧 MP4Merger.setup() - Video init flush completed');
      if (file.moov) {
        console.log('🔧 MP4Merger.setup() - Video init has moov box');
        ...
        console.log('🔧 MP4Merger.setup() - Video init segment has no moov box, likely WebM format');
        throw new Error('Video is not an mp4!');
      }
    }

    if (audioDuration) {
      console.log('🔧 MP4Merger.setup() - Processing audio init segment');
      // Check if this is a WebM file (starts with 0x1A)
      const firstByte = new Uint8Array(audioInitSegment)[0];
      console.log('🔧 MP4Merger.setup() - Audio init first byte:', firstByte, '(WebM signature is 0x1A = 26)');
      if (firstByte === 0x1A) {
        console.log('🔧 MP4Merger.setup() - Detected WebM init segment, skipping MP4Box processing');
      ...
      if (file.moov) {
        ...
        console.log('🔧 MP4Merger.setup() - Audio init segment has no moov box, likely WebM format');
        throw new Error('Audio is not an mp4!');
      }
    }
  ...
  async convert(videoDuration, videoInitSegment, audioDuration, audioInitSegment, zippedFragments) {
    console.log('🔧 MP4Merger.convert() called with fragments:', zippedFragments.length);
    this.setup(videoDuration, videoInitSegment, audioDuration, audioInitSegment);
    console.log('🔧 MP4Merger.setup() completed');

    let lastProgress = 0;
    for (let i = 0; i < zippedFragments.length; i++) {
      console.log(`🔧 MP4Merger: Processing fragment ${i}/${zippedFragments.length}`);
      if (this.cancelled) {
        this.destroy();
        this.blobManager.close();
        throw new Error('Cancelled');
      }
      console.log(`🔧 MP4Merger: Fragment ${i} track: ${zippedFragments[i].track}`);
      if (zippedFragments[i].track === 0) {
        console.log(`🔧 MP4Merger: Pushing video fragment ${i}`);
        await this.pushFragment(this.videoTrack, zippedFragments[i]);
        console.log(`🔧 MP4Merger: Video fragment ${i} pushed successfully`);
      } else {
        console.log(`🔧 MP4Merger: Pushing audio fragment ${i}`);
        await this.pushFragment(this.audioTrack, zippedFragments[i]);
        console.log(`🔧 MP4Merger: Audio fragment ${i} pushed successfully`);
      }
    ...
    console.log('🔧 MP4Merger: All fragments processed, calling finalize()');
    const blob = await this.finalize();
    console.log('🔧 MP4Merger: finalize() completed, blob size:', blob.size);
    this.destroy();
